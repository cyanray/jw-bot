...
create unique index if not exists user_info_qq_uindex
    on user_info (qq);
		...
		SQLite::Database db_(DB_FILE_NAME, SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
		db_.exec(sql_str);
	}
	...
	public:
		...
		string GetFriendSid(QQ_t qq)
		{
			SQLite::Statement sql(db_, "SELECT sid FROM user_info WHERE qq = (SELECT friend_qq FROM user_info WHERE qq = ?) AND friend_qq = ?;");
			sql.bind(1, qq.ToInt64());
		...
		QQ_t GetFriendQQ(QQ_t qq)
		{
			SQLite::Statement sql(db_, "SELECT qq FROM user_info WHERE qq = (SELECT friend_qq FROM user_info WHERE qq = ?) AND friend_qq = ?;");
			sql.bind(1, qq.ToInt64());
			sql.bind(2, qq.ToInt64());
			if (sql.executeStep())
			{
				return QQ_t( (int64_t)sql.getColumn(0) );
			}
			else
			{
				return QQ_t();
		...
		int UpdateFriendQQ(QQ_t qq, QQ_t friend_qq)
		{
			SQLite::Statement sql(db_, "UPDATE user_info SET friend_qq = ? WHERE qq = ?");
			sql.bind(1, friend_qq.ToInt64());
			sql.bind(2, qq.ToInt64());
			return sql.exec();
		}
